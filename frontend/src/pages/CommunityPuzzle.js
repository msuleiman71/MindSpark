...
import { ArrowLeft, Lightbulb, RotateCcw, Heart, Coins, Star as StarIcon } from 'lucide-react';
...
const CommunityPuzzle = () => {
  ...
  const { hints, setHints, lives, coins, addCoins } = useGame();
  
  const [puzzle, setPuzzle] = useState(location.state?.puzzle || null);
  ...
  const loadPuzzle = async () => {
    try {
      const data = await communityAPI.getPuzzle(id);
      setPuzzle(data);
    } catch (error) {
  ...
  const handleSuccess = () => {
    ...
    const stars = calculateStars(timeTaken, attempts);
    addCoins(stars * 10);
    setShowSuccess(true);
  };

  const calculateStars = (time, attemptCount) => {
    if (time < 30 && attemptCount === 0) return 3;
    if (time < 60 && attemptCount <= 1) return 2;
    return 1;
  };
  ...
  const handleSubmitRating = async () => {
    ...
    if (rating > 0) {
      try {
        await communityAPI.ratePuzzle(id, rating, review || null);
      } catch (error) {
  ...
  const renderPuzzle = () => {
    ...
    const puzzleData = {
      ...puzzle.puzzle_data,
      hint: puzzle.puzzle_data.hint || puzzle.hint,
      explanation: puzzle.puzzle_data.explanation || puzzle.explanation
    };
    ...
    switch (puzzle.puzzle_data.type) {
      case 'tap':
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-400 via-cyan-400 to-teal-400 flex flex-col">
      ...
      <div className="px-2 sm:px-3 md:px-4 pb-3 sm:pb-4 md:pb-6">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-xl sm:rounded-2xl md:rounded-3xl shadow-xl p-3 sm:p-4 md:p-6 text-center">
            ...
            <p className="text-xs sm:text-sm text-gray-500 mt-2">by {puzzle.creator_name}</p>
          </div>
        </div>
      </div>

      {/* Hint Display */}
      {showHint && puzzle.puzzle_data.hint && (
      ...
      {/* Success Modal */}
      {showSuccess && !showRating && (
        <SuccessModal
          level={0}
          explanation={puzzle.puzzle_data.explanation || puzzle.explanation || 'Great job!'}
          onNext={handleNext}
          onLevels={() => navigate('/community-puzzles')}
      ...
      {showRating && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 space-y-6">
            <h3 className="text-2xl font-bold text-gray-800 text-center">Rate this puzzle</h3>
...
export default CommunityPuzzle;
